  ...
  <div class="h-screen w-width bg-gray-300 flex flex-col justify-center items-center">
        ...
        <div class="todos w-1/2 mt-8">
            <div class="pendings" v-if="totalPendings>0">
                ...
                <ul id="pendingList">
                    <li 
                    v-for="(pending, index) in pendingList" :key="index"
                    @click="moveToCompleted(pending, index)"
                    class="text-center p-3 bg-white mt-4 rounded shadow-lg cursor-pointer hover:bg-blue-500">
                    {{pending}}
                    </li>
                </ul>
            </div>
            <div class="completed mt-8" v-if="totalCompleted>0">
                ...
                <ul id="completedList">
                    <li 
                    v-for="(complete, index) in completedList" :key="index"
                    @click="moveToPending(complete, index)"
                    class="text-center p-3 bg-white line-through text-yellow-500 mt-4 rounded shadow-lg cursor-pointer hover:bg-purple-500 hover:text-white">
                    {{complete}}
                    </li>
                </ul>
            </div>
